...
x4 = x3;
x5 = outer_diameter/2 + dx*2/3;
x6 = outer_diameter/2 - dx*1/3;
x7 = inner_diameter/2 + dx*1/3;
x8 = inner_diameter/2 - dx*2/3;
x9 = x0;
...
y5 = x5 <= y4 ? y4 : y4 + (x5-x4)/tan(overhang);
y6 = y5 + height;
y7 = y6;
y8 = y5;
y9 = y8 - abs((x8-x9)/tan(overhang));
...
module profile(){
  ...
  difference(){
    ...
    // chamfers
    for(i = [0,1])
    translate(radiiPoints[i])
    circle(r=0.7, $fn=4);

    for(i = [6,7])
    translate(radiiPoints[i])
    rotate(i % 2 ? -45 : 45)
    square([sqrt(2)*chamfer, 2*chamfer], center=true);
  }
}
